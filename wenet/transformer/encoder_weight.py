...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        self.feature_weight = nn.Parameter(torch.ones(self.num_blocks))
    ...
    def forward(
        ...
        xs = xs.reshape(xs.shape[0], xs.shape[1], self.output_size(), self.num_blocks)
        norm_weights = F.softmax(self.feature_weight, dim=-1)
        xs = xs.matmul(norm_weights)
