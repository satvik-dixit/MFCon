...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        self.ln_1 = torch.nn.LayerNorm(output_size, eps=1e-12)
        self.ln_2 = torch.nn.LayerNorm(output_size, eps=1e-12)
        self.ln_3 = torch.nn.LayerNorm(output_size, eps=1e-12)
        self.ln_4 = torch.nn.LayerNorm(output_size, eps=1e-12)
        self.ln_5 = torch.nn.LayerNorm(output_size, eps=1e-12)
        self.ln_6 = torch.nn.LayerNorm(output_size, eps=1e-12)
    ...
    def forward(
        ...
        cat_outputs = torch.cat(out, dim=-1)

        ln_out = []
        ln_out.append(self.ln_1(out[0]))
        ln_out.append(self.ln_2(out[1]))
        ln_out.append(self.ln_3(out[2]))
        ln_out.append(self.ln_4(out[3]))
        ln_out.append(self.ln_5(out[4]))
        ln_out.append(self.ln_6(out[5]))
            
        ln_cat_outputs = torch.cat(ln_out, dim=-1)

        if self.normalize_before:
            xs = self.after_norm(cat_outputs)
        # Here we assume the mask is not changed in encoder layers, so just
        # return the masks before encoder layers, and the masks will be used
        # for cross attention with decoder later
        return xs, ln_cat_outputs
