...
from loss import softmax, amsoftmax, contrastive_amsoftmax
# import datetime
...
class Task(LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        elif self.hparams.loss_name == "contrastive_amsoftmax_supcon_outputs_all":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='supcon', use_outputs=True, all_blocks=True)
        elif self.hparams.loss_name == "contrastive_amsoftmax_ntxent_outputs_all":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='ntxent', use_outputs=True, all_blocks=True)
        elif self.hparams.loss_name == "contrastive_amsoftmax_triplet_outputs_all":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='triplet', use_outputs=True, all_blocks=True)
        elif self.hparams.loss_name == "contrastive_amsoftmax_supcon_outputs_false":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='supcon', use_outputs=False, all_blocks=True)
        elif self.hparams.loss_name == "contrastive_amsoftmax_ntxent_outputs_false":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='ntxent', use_outputs=False, all_blocks=True)
        elif self.hparams.loss_name == "contrastive_amsoftmax_triplet_outputs_false":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='triplet', use_outputs=False, all_blocks=True)
        elif self.hparams.loss_name == "contrastive_amsoftmax_supcon_false_false":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='supcon', use_outputs=False, all_blocks=False)
        elif self.hparams.loss_name == "contrastive_amsoftmax_ntxent_false_false":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='ntxent', use_outputs=False, all_blocks=False)
        elif self.hparams.loss_name == "contrastive_amsoftmax_triplet_false_false":
            self.loss_fun = contrastive_amsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes, loss_type='triplet', use_outputs=False, all_blocks=False)
        else:
    ...
    def forward(self, x):
        ...
        embedding, outputs = self.encoder(feature)
        return embedding
